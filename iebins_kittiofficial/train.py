...
from utils import post_process_depth, flip_lr, silog_loss, compute_errors, compute_errors_kb, eval_metrics, entropy_loss, colormap, \
...
def online_eval(model, dataloader_eval, gpu, epoch, ngpus, group, post_process=False):
    eval_measures = torch.zeros(11).cuda(device=gpu)  #为什么是11？
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            pred_depths_r_list, _, _ = model(image) #代码区别
            if post_process:
...
def main():
    ...
    if save_files:
        ...
        command = 'cp iebins_kittiofficial/train.py ' + aux_out_path
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + ' && cp iebins_kittiofficial/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + ' && cp iebins_kittiofficial/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
